...
int main(int argc, char const *argv[]) {
  ...
  demande = NULL;
  mqd_t* mq_list;
  mq_list = init(listOfBot, nbBot, mapOfGame.spawn, mapOfGame.nbSpawn, server, demande, buffer, taille);
  mq_unlink("/server");
  return 0;
}
  ...
  //recuperation des client et creation des robots
    for (int i = 0; i < nb_bot; i++) {
      ...
      if(demande->action == 1){
        ...
        mq_send(mq_list[i], (char*) &bot_list[i].pos, sizeof(coord), 1);
        mq_send(mq_list[i], (char*) &bot_list[i].id, sizeof(char), 1);
      }
      buffer = NULL;
      buffer = malloc(taille);
    }
